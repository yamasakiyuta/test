...
from cms.forms import BookForm, ImpressionForm, SearchForm
...
def home(request):
    ...
    if request.GET.get('your_name'):
        ...
        books = Book.objects.filter(func_name__contains=request.GET.get('your_name'))
    else:
...
def hello_get_query(request):
    d = {'your_name': request.GET.get('your_name')}
    return render(request, 'cms/get_query.html', d)


def book_list2(request):
    """書籍の一覧"""
    books=Book.objects.filter(func_name__contains='your_name')
    return render(request,'cms/book_list.html',{'books': books})
